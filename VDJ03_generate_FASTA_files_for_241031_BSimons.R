...
##### METADATA
#####----------------------------------------------------------------------#####
mid.metadata <- readxl::read_excel("/media/hieunguyen/HNSD01/src/sc_bulk_BCR_data_analysis/preprocessing/241031_BSimons/241031_sample_sheet.xlsx")

sample.list <- list()
for (mouse.id in unique(mid.metadata$mouse)){
  sample.list[[mouse.id]] <- to_vec(for (item in subset(mid.metadata, mid.metadata$mouse == mouse.id)$MID) sprintf("MID%s", item))
}
...
for (mouse.id in names(sample.list)){
  input.clonesets <- subset(clonesets, clonesets$id %in% sample.list[[mouse.id]])
  ...
  output.all.fasta <- generate_fasta(clonesets = input.clonesets, 
                                     mouse.id = mouse.id,
                                     path.to.save.output = file.path(path.to.save.fasta, mouse.id), 
                                     ref.gene = ref.gene, 
