...
# path.to.14.output <- file.path(outdir, "VDJ_output", "14_output_20250323", paste(list.of.PROJECT, collapse = "_"))
path.to.14.output <- file.path(outdir, "VDJ_output", "14_output_20250323", paste(sc.projects[[1]], collapse = "_"))
...
all.files <- Sys.glob(file.path(path.to.tree.05.output, "*.csv"))
...
public.clonedf <- rbind(data.frame(cloneCount = 0, id = "START", SampleID = "publicClone", Freq = 0, accum.Freq = 0), 
                        public.clonedf)
...
#####----------------------------------------------------------------------#####
path.to.save.svg <- file.path(path.to.14.output, sprintf("%s.svg", sc.projects[[1]]))
