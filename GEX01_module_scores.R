...
for (input.dataset in names(path.to.all.s.obj)){
  ...
  if (file.exists(file.path(path.to.01.output, sprintf("finished_%s", input.dataset))) == FALSE){
    ...
    write.csv(data.frame(status = c("finished")), file.path(path.to.01.output, sprintf("finished_%s", input.dataset)))    
  } else {
