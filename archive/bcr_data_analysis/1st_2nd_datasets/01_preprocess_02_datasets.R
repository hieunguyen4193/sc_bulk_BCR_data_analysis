...
path.to.main.input <- file.path(outdir, PROJECT, "1st_round") # inputs taken from the downstream pipeline
...
for (sample in second_dataset){
  ...
  tmp.s.obj <- readRDS(file.path(all.samples[[sample]], "s8a_output", sprintf("%s.output.s8a.rds", PROJECT)))
