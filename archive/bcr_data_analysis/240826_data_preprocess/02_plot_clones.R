...
for (mouse.id in unique(mid.metadata$mouse)){
  ...
  clonesetsdf <- readxl::read_excel(file.path(path.to.01.output, sprintf("clonesets_%s.split_clones_%s.xlsx", mouse.id, thres)))
  ...
  #####----------------------------------------------------------------------#####
  path.to.save.plot <- file.path(path.to.02.output, "barplot", mouse.id)
  ...
  ##### chord diagram
  #####----------------------------------------------------------------------#####
  path.to.save.plot <- file.path(path.to.02.output, "chord_diagram", mouse.id)
  dir.create(path.to.save.plot, showWarnings = FALSE, recursive = TRUE)
  ...
  #####----------------------------------------------------------------------#####
  path.to.save.plot <- file.path(path.to.02.output, "MHI_heatmap", mouse.id)
  dir.create(path.to.save.plot, showWarnings = FALSE, recursive = TRUE)
  if (length(all.mids) > 1){
  ...
  #####----------------------------------------------------------------------#####
  path.to.save.plot <- file.path(path.to.02.output, "alluvial_tracking", mouse.id)
  dir.create(path.to.save.plot, showWarnings = FALSE, recursive = TRUE)
  if (length(all.mids) > 1){
    for (input.mid in all.mids){
