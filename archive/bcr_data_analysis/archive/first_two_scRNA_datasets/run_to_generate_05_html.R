...
path.to.rmd <- file.path(path.to.project.src, "05_trajectory_analysis_with_monocle3.Rmd")
...
for (dataset.name in c("1st_dataset_removed_7_9_removed_16_removed_9.with_reInt.res1",
  ...
  save.html.name <- sprintf("%s.monocle3.html", dataset.name)
  if (file.exists(file.path(path.to.save.html, save.html.name)) == FALSE){
    rmarkdown::render(input = path.to.rmd, 
                      ...
                      params = list(
                        ...
                        candidate.root.node = candidate.root.node
                      ))  
  }  
}
