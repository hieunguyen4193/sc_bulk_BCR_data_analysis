...
for (filter.ht.type in c("remove_last_ht", sprintf("cutoff_%s", cut.off.ht))){
  ...
  path.to.05.output <- file.path(outdir, "VDJ_output", sprintf("05_output_filterHT_%s", filter.ht.type), paste(list.of.PROJECT, collapse = "_"))
  ...
  if (file.exists(file.path(path.to.05.output, circos.group.type, "all_data.rds")) == FALSE){
    ...
    for (PROJECT in list.of.PROJECT){
      ...
      dir.create(file.path(path.to.VDJ.output, circos.group.type), showWarnings = FALSE, recursive = TRUE)
      ...
      for (mid in unique(full.clonedf$id)){
        if (file.exists(file.path(path.to.VDJ.output, 
                                  circos.group.type, 
          ...
          print(sprintf("File exists at %s, reading in ...", 
                        file.path(path.to.VDJ.output, circos.group.type, sprintf("%s.simplified.csv", mid))))
          all.data[[sprintf("%s_%s", PROJECT, mid)]] <- read.csv(file.path(path.to.VDJ.output, circos.group.type, sprintf("%s.simplified.csv", mid)), sep = "\t")
        }
      }
