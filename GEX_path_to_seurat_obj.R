outdir <- "/media/hieunguyen/GSHD_HN01/outdir"
path.to.all.s.obj <- list(
  `1st_dataset` = file.path(outdir, "sc_bulk_BCR_data_analysis_v0.1/1st_2nd_BSimons_Datasets/data_analysis/01_output/2nd_dataset_removed_5_6.without_reInt.res1/2nd_dataset_removed_5_6.without_reInt.res1.rds"),
  `2nd_dataset` = file.path(outdir, "sc_bulk_BCR_data_analysis_v0.1/1st_2nd_BSimons_Datasets/data_analysis/01_output/1st_dataset_removed_7_9_removed_16_removed_9.with_reInt.res1/1st_dataset_removed_7_9_removed_16_removed_9.with_reInt.res1.rds"),
  `241002_BSimons` = file.path(outdir, "sc_bulk_BCR_data_analysis_v0.1/241002_BSimons/data_analysis/03_output/quantile_0.85/PP3/GEX_sample_PP3_seurat_object.rds"),
  `240805_BSimons` = file.path(outdir, "sc_bulk_BCR_data_analysis_v0.1/240805_BSimons/data_analysis/04_output/quantile_0.85/all_samples/s8_output/240805_BSimons.output.s8.rds"),
  `241104_BSimons` = file.path(outdir, "sc_bulk_BCR_data_analysis_v0.1/241104_BSimons/data_analysis/03_output/quantile_0.85/PP7/GEX_sample_PP7_seurat_object.rds")
)

