...
## Circos plot
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
clone.sampledf <- data.frame(SampleID = unique(clonedf$name))
for (sample2 in unique(clonedf$name)){
  clone.sampledf[[sample2]] <- unlist(
    lapply(
      clone.sampledf$SampleID, function(sample1){
        return(length(unique(intersect(subset(clonedf, clonedf$name == sample1)$VJcombi_CDR3_0.85, subset(clonedf, clonedf$name == sample2)$VJcombi_CDR3_0.85))))
      }
    )
  )
}
