...
if (file.exists(file.path(path.to.04.output, sprintf("clone_Table_%s.xlsx", integration.case))) == FALSE){
  ...
  writexl::write_xlsx(clonedf, file.path(path.to.04.output, sprintf("clone_Table_%s.xlsx", integration.case)))
} else {
  clonedf <- readxl::read_excel(file.path(path.to.04.output, sprintf("clone_Table_%s.xlsx", integration.case)))
}
```
