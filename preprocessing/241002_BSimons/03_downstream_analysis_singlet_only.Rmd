...
s.obj <- readRDS(file.path(path.to.main.input, 
                           ...
                           sprintf("%s_1st_round", sample.id), 
                           "s8a_output", sprintf("%s.output.s8a.rds", PROJECT)))

raw.s.obj <- readRDS(file.path(path.to.main.input,
                               sprintf("%s_round", analysis.round), 
                               sprintf("%s_1st_round", sample.id), "s1_output",
                               sprintf("%s.output.s1.rds", PROJECT)))
...
yfp.cells <- names(yfp.exprs[yfp.exprs != 0])
nonyfp.cells <- names(yfp.exprs[yfp.exprs == 0])

meta.data <- s.obj@meta.data %>% rownames_to_column("barcode") %>%
  rowwise() %>%
  mutate(YFP = ifelse(barcode %in% yfp.cells, "YFP+", "YFP-")) %>%
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
print(sprintf("number of YFP cells: %s", length(yfp.cells)))
```

## UMAP: all clusters in the sample, highlighted YFP cells
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
DimPlot(object = s.obj, reduction = "RNA_UMAP", cells.highlight = yfp.cells, label = TRUE, label.box = TRUE, label.size = 10, repel = TRUE) + 
...
## Check expression of Aicda gene in all cells

### UMAP
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
FeaturePlot(object = s.obj, reduction = "RNA_UMAP", label = TRUE, features = c("Aicda"))
```

### Violin plot
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = s.obj, features = c("Aicda"))
```

### In each hashtags
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = s.obj, features = c("Aicda"), split.by = "HTO_classification")
```

## Check expression of Aicda gene in YFP+ cells

### UMAP
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
FeaturePlot(object = subset(s.obj, cells = yfp.cells), reduction = "RNA_UMAP", label = TRUE, features = c("Aicda"))
```

### Violin plot
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = subset(s.obj, cells = yfp.cells), features = c("Aicda"))
```

### In each hashtags
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = subset(s.obj, cells = yfp.cells), features = c("Aicda"), split.by = "HTO_classification")
```
...
### UMAP
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
FeaturePlot(object = subset(s.obj, cells = nonyfp.cells), reduction = "RNA_UMAP", label = TRUE, features = c("Aicda"))
```

### Violin plot
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = subset(s.obj, cells = nonyfp.cells), features = c("Aicda"))
```

### In each hashtags
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = subset(s.obj, cells = nonyfp.cells), features = c("Aicda"), split.by = "HTO_classification")
```

## Number of YFP +- cells in each hashtags
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
table(s.obj$YFP, s.obj$HTO_classification) %>% data.frame() %>% pivot_wider(names_from = "Var1", values_from = "Freq") %>% create_dt()
```

## Number of YFP +- cells in each hashtags
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
table(s.obj$YFP, s.obj$seurat_clusters) %>% data.frame() %>% pivot_wider(names_from = "Var1", values_from = "Freq") %>% create_dt()
```


```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
count.mat <- GetAssayData(object = s.obj, slot = "counts", assay = "RNA")[c("Aicda"), yfp.cells]
meta.data <- s.obj@meta.data %>% 
  rownames_to_column("barcode") %>% 
  subset(YFP == "YFP+") %>% 
  subset(select = c(barcode, seurat_clusters)) %>%
  rowwise() %>% 
  mutate(Aicda = count.mat[[barcode]])
