...
path.to.02.output <- file.path(path.to.output, "02_output")
dir.create(path.to.02.output, showWarnings = FALSE, recursive = TRUE)

#### list of output paths
path.to.clone.distribution <- file.path(path.to.02.output, "clone_distribution_UMAP")
...
```{r echo=FALSE, warning=FALSE, results='asis', message=FALSE, fig.width=14, fig.height=10}
gene.list <- c("Cxcr3", "Tigit", "Ly6a", "Cd93", "H2-Aa", "H2-Ab", "Epcam", "Slamf6", "Sdc1")
```

## UMAP {.tabset}
```{r echo=FALSE, warning=FALSE, results='asis', message=FALSE, fig.width=14, fig.height=10}
DefaultAssay(s.obj) <- "RNA"
for (gene in intersect(gene.list, row.names(s.obj))){
  cat(sprintf("### Gene %s \n", gene))
  p <- FeaturePlot(object = s.obj, features = c(gene), reduction = "INTE_UMAP", label = TRUE)
  print(p)
  cat("\n \n")
}
```
...
for (gene in intersect(gene.list, row.names(s.obj))){
  cat(sprintf("### Gene %s \n", gene))
  p <- VlnPlot(object = s.obj, features = c(gene))
  print(p)
  cat("\n \n")
}
```
...
for (gene in intersect(gene.list, row.names(s.obj))){
  cat(sprintf("### Gene %s \n", gene))
  p <- VlnPlot(object = s.obj, features = c(gene), group.by = "YFP.group")
  print(p)
  cat("\n \n")
}
```
